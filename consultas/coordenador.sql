SELECT 
	COORD.coord_nome AS COORDENADOR,	
	C.NOME AS CURSO,
	TD.CODTURMA,
...
FROM OPENQUERY (
	HOSTGATOR, 
	'SELECT * FROM sur_question LEFT JOIN sur_response ON(resp_quest_id = quest_id) WHERE periodo_id = 15 AND resp_cat_id = 2 AND resp_quest_id <> 14 '
) CPA
...
INNER JOIN OPENQUERY (HOSTGATOR,'SELECT * FROM sur_coordenador') COORD ON(
	COORD.coord_cod_curso = C.CODCURSO COLLATE SQL_Latin1_General_CP1_CI_AI
	AND COORD.coord_ativo = 1
)
WHERE MD.IDPERLET = 113
AND C.CODTIPOCURSO = 1 
AND CODMODALIDADECURSO = 1
GROUP BY 
	COORD.coord_nome,
	C.CODCURSO,
	C.NOME,
