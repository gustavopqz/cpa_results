...
import dados.dados_coordenadores as dados
import codecs

#Configuração da página
st.set_page_config(
    page_title="CPA - COORDENADORES",
...
df = pd.read_excel(
    io='dados/unef.xlsx',
    engine='openpyxl',
    sheet_name='coordenador'
)
# IMPORTAÇÃO DA PLANILHA PERGUNTAS
df_perguntas = pd.read_excel(
    io='dados/unef.xlsx',
    engine='openpyxl',
    sheet_name='perguntas_coordenador'
)
df_perguntas.set_index('id', inplace=True)
# CABEÇALHO DO FILTRO
st.sidebar.header("Filtros")

# CARREGA FILTRO COORDENADOR 
coordenador = st.sidebar.multiselect(
    "Coordenador",
   options=df["COORDENADOR"].unique()
)

# TRATA FILTRO TURMAS DE ACORDO COM O COORDENADOR SELECIONADO
if coordenador:
    coord_select = df.query(
    "(COORDENADOR == @coordenador)"
    )
    curso = st.sidebar.multiselect(
        "Curso",
    options=coord_select["CURSO"].unique()
    )
else:
    curso = st.sidebar.multiselect(
        "Curso",
...
if not (coordenador) and not (curso):
    ...
    df_selection = df.query(
    "(COORDENADOR == @coordenador & CURSO == @curso)"
    )
    dtframe = dados.detalhe_perguntas(df_selection)
    total_excelente = int(df_selection["EXCELENTE"].sum())
    total_otimo = int(df_selection["ÓTIMO"].sum())
...
st.markdown('AVALIAÇÃO POR QUANTIDADE')
